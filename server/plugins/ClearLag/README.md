...
* **Dry-Run:** `--preview` reports what a clear would remove, without removing anything.
* **Villager AI Throttle:** Suspends villager AI while no player is nearby, and wakes it instantly on interaction.
* **Anti-Crash Item Limits:** Rejects items with crash-sized NBT, names, lore, books or nested containers.
* **Metric History:** Records TPS/RAM/entity history to SQLite and reports it with `/clearlag history`.

## Main Commands
* `/clearlag`: Shows the main help menu.
* `/clearlag clear <items/entity/all> [--preview]`: Manually clears entities; `--preview` only reports what would go.
* `/clearlag history <30m|24h|7d>`: Shows recorded performance history.
